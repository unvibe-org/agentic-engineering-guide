## Instructors

Not academics — practitioners. All three ship production code with AI daily.

```{=typst}
#v(0.8em)
#grid(
  columns: (52pt, 1fr),
  column-gutter: 14pt,
  box(clip: true, radius: 50%, width: 52pt, height: 52pt)[
    #image("assets/adam-photo.cmyk.jpg", width: 52pt, height: 52pt, fit: "cover")
  ],
  [
    #text(fill: rgb("#E8E4DF"), weight: 600)[Adam Charnock]
    #v(0.2em)
    #text(fill: rgb("#DA7756"), size: 7.5pt)[#link("https://www.linkedin.com/in/adamcharnock/")[linkedin.com/in/adamcharnock]]
    #v(0.3em)
    #text(size: 8pt)[Founder of Lithus, where he manages bare-metal Kubernetes infrastructure for SMEs. Previously contracted at Twitter and Blocknative. Adam has run production systems at scale for 18 years — he teaches how to calibrate control and let agents handle the right things.]
  ]
)

#v(1.0em)
#line(length: 100%, stroke: 0.5pt + rgb("#ffffff14"))
#v(1.0em)

#grid(
  columns: (52pt, 1fr),
  column-gutter: 14pt,
  box(clip: true, radius: 50%, width: 52pt, height: 52pt)[
    #image("assets/dominik.cmyk.jpg", width: 52pt, height: 52pt, fit: "cover")
  ],
  [
    #text(fill: rgb("#E8E4DF"), weight: 600)[Dominik Grusemann]
    #v(0.2em)
    #text(fill: rgb("#DA7756"), size: 7.5pt)[#link("https://www.linkedin.com/in/dominikgrusemann/")[linkedin.com/in/dominikgrusemann]]
    #v(0.3em)
    #text(size: 8pt)[Co-Founder of Marbles AI and former CTO of Chatchamp (acquired 2023). Previously led BMW's autonomous driving department toward agile. Dominik focuses on CLAUDE.md best practices and compound engineering.]
  ]
)

#v(1.0em)
#line(length: 100%, stroke: 0.5pt + rgb("#ffffff14"))
#v(1.0em)

#grid(
  columns: (52pt, 1fr),
  column-gutter: 14pt,
  box(clip: true, radius: 50%, width: 52pt, height: 52pt)[
    #image("assets/traun_profile_photo.cmyk.jpg", width: 52pt, height: 52pt, fit: "cover")
  ],
  [
    #text(fill: rgb("#E8E4DF"), weight: 600)[Traun Leyden]
    #v(0.2em)
    #text(fill: rgb("#DA7756"), size: 7.5pt)[#link("https://www.linkedin.com/in/tleyden/")[linkedin.com/in/tleyden]]
    #v(0.3em)
    #text(size: 8pt)[Co-Founder of Fluensy.app. Ex-Databricks / Couchbase senior engineer with 20+ years shipping backend systems in Go, Python, and Rust. Traun brings deep experience in building reliable, well-tested software — and knows exactly where AI-generated code breaks down in production.]
  ]
)
```
